...
В рамках данной функции каждому целочисленному значению однозначно соответствует его квадрат. Так вот чистые функции в Haskell - это и есть функции в математическом смысле. Они представляют собой описание того, как входное значение определяет выходное значение.
...
```
Это - очень важное свойство чистых функций: мы всегда можем безопасно заменить места их вызова соответствующими возвращённым значениями, и работа приложения при этом гарантированно останется неизменной. Именно поэтому работать с чистой функцией легко.
...
Теперь псевдоним `specValue` задан в `let`-выражении, и мы хотим использовать его в `where`-выражении. Увы и ах, но такой код не скомпилируется, и мы увидим ошибку:

```bash
Not in scope: ‘specValue’
```

Это и есть то самое ограничение: мы не можем использовать `let`-псевдонимы внутри `where`-выражения.
...
* В теле ЧФ можно ввести одно или более локальных выражений, с помощью ключевых слов `where` и `let`.
