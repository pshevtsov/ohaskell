...
* Чистые функции (далее ЧФ) - это функции в математическом смысле слова: они всегда что-то принимают и всегда что-то возвращают. Кроме того, ЧФ не могут иметь побочных эффектов и не могут взаимодействовать с внешним миром.
* `f x` - это вызов функции `f` с аргументом `x`, или применение функции `f` к значению `x`.
* Мы всегда можем безопасно заменить вызов ЧФ возвращаемым ею значением, и у нас ничего не сломается.
* Мы не можем явно указать точку выхода из ЧФ, потому что применительно к ЧФ это не имеет смысла.
