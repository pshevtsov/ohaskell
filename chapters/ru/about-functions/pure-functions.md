...
То есть функция представляет собою закон, определяющий то, *как* одному конкретному значению соответствует другое конкретное значение. Например, уже известная нам функция `f1`: 
